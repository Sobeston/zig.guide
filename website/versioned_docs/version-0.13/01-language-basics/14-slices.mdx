...
:::go
Slices in Zig are in many ways similar to slices in Go, and use similar syntax
(e.g. `array[start..end]` over `array[start:end]`). However, there is one very
important diference.

In Go, there is no explicit ownership or memory management. This means that the
data that a slice points to is effectively "owned" by the garbage collector.
However in Zig, a slice is roughly what go calls a "slice header" (i.e. the
struct that contains the pointer, length, and in Go's case, the cap). This has
important implications:

- Zig slices do not have a Cap field, as they will not automatically resize -
  they are not tied to allocation.
